...
**We use the following set of confounders in this analysis: `Trauma.center`, `Cardiac.arrest.ph`, `SBP.ph.min, DBP.ph.min, HR.ph.max`, `SBP.ph, DBP.ph, HR.ph`, `Shock.index.ph` (`HR.ph`/`SBP.ph`), `Cristalloid.volume`, `Colloid.volume`, `SpO2.ph.min`, `Vasopressor.therapy`, `HemoCue.init`, `Delta.hemoCue`, `Delta.shock.index`, `AIS.external`.**
...
rawData <- read.csv("data/4_computed_dataset.csv",   na.strings = c("", "NR", "NA", "NF","IMP", "ND"),sep = ",")
...
selectedData$Tranexamic.acid <- sapply(selectedData$Tranexamic.acid, function(x) dplyr::if_else(is.na(x), 0, as.double(as.character(x))))
selectedData$Tranexamic.acid <- as.factor(selectedData$Tranexamic.acid)

selectedData$TBI <- sapply(selectedData$TBI, function(x) dplyr::if_else(is.na(x), 0, as.double(as.character(x))))
selectedData$TBI <- as.factor(selectedData$TBI)

selectedData$Death <- sapply(selectedData$Death, function(x) dplyr::if_else(is.na(x), 0, as.double(as.character(x))))
selectedData$Death <- as.factor(selectedData$Death)

selectedData$Neurosurgery.day0 <- sapply(selectedData$Neurosurgery.day0, function(x) dplyr::if_else(is.na(x), 0, as.double(as.character(x))))
...
# Save the data
```{r save_data_for_ate}
write.csv(data_indAll, file="~/Documents/Polytechnique/MAP573-Data_Analysis_and_Unsupervised_Learning/Project/HTE/data/data_preprocessed_all_individuals.csv")
write.csv(data_indTBI, file="~/Documents/Polytechnique/MAP573-Data_Analysis_and_Unsupervised_Learning/Project/HTE/data/data_preprocessed_tbi_individuals.csv")
```